...
exp4 = 'column_test_rrtm_ozone_old'
...
ds4 = af.open_experiment(exp4, 10, 10, 'atmos_monthly.nc', variables=['temp','ozone'], cal=False)
...
plt.title('RRTM+SBM SCM EXPERIMENT', fontsize=15)
#plt.plot(np.squeeze(ds3.temp.mean('time').values)[1:], ds3.pfull.values[1:], label='no ozone', linewidth=3)
plt.plot(np.squeeze(ds4.temp.mean('time').values)[1:], ds4.pfull.values[1:], label='ozone', linewidth=3)
...
#plt.figure(3)
#plt.plot(ds3.time.values, np.squeeze(ds3.coszen.values)*1360., label='rrtm_noinputfile')
#plt.legend(loc='best')


plt.figure(4)
plt.plot(np.squeeze(ds4.ozone.mean('time').values), ds4.pfull.values, label='rrtm_ozoneinputfile')
plt.legend(loc='best')
