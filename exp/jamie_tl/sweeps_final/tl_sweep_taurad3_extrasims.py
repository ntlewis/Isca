...
namelist = Namelist({
    ...
    'hs_forcing_nml': {
        ...
        'ka':   -8.,#*32*np.sqrt(2),      # Constant Newtonian cooling timescale (default 40 days)
        'ks':    -2.,#*32*np.sqrt(2),      # Boundary layer dependent cooling timescale (default 4 days)
...
if __name__ == '__main__':
    ...
    # set 1 
    taus1 = np.array([2., 1., 1./2.])
    rads1 = np.array([2.*np.sqrt(2.), np.sqrt(2.), 1./np.sqrt(2.), 1./(2.*np.sqrt(2.))])
    names1 = ['2_1tau','1_1tau','1_2tau']
    radnames1 = ['2rt2_1rad','1rt2_1rad','1_1rt2rad','1_2rt2rad']
    steps1 = np.array([600.,600.,600.,300.])
    
    # set 2
    taus2 = np.array([2.*np.sqrt(2.), np.sqrt(2.), 1./np.sqrt(2.), 1./(2.*np.sqrt(2.))])
    rads2 = np.array([2., 1., 1./2.])
    names2 = ['2rt2_1tau','1rt2_1tau','1_1rt2tau','1_2rt2tau']
    radnames2 = ['2_1rad','1_1rad','1_2rad']
    steps2 = np.array([600.,600.,300.])

    # run set 1
    for n, tau in enumerate(taus1):
        for m, rad in enumerate(rads1):
            omega_exp = omega_e / om
            rad_exp = radius * rad
            ka_exp = -8. * tau 
            ks_exp = -2. * tau 
            step_exp = steps1[m]
            run_exp = exp.derive('F_tl_'+radnames1[m]+'_'+'1_64om_'+names1[n]+'_rad')
    ...
    for n, tau in enumerate(taus2):
        for m, rad in enumerate(rads2):
            omega_exp = omega_e / om
            rad_exp = radius * rad
            ka_exp = -8. * tau 
            ks_exp = -2. * tau 
            step_exp = steps2[m]
            run_exp = exp.derive('F_tl_'+radnames2[m]+'_'+'1_64om_'+names2[n]+'_rad')
            run_exp.namelist['main_nml']['dt_atmos'] = step_exp
            run_exp.namelist['constants_nml']['omega'] = omega_exp
            run_exp.namelist['constants_nml']['radius'] = rad_exp 
            run_exp.namelist['hs_forcing_nml']['ka'] = ka_exp 
