...
NCORES = 24
RESOLUTION = 'T63', 25  # T42 horizontal resolution, 25 levels in pressure
...
namelist = Namelist({
    ...
    'hs_forcing_nml': {
        't_zero': 285.,    # temperature at reference pressure at equator (default 315K)
        'delh': 0.6,       # equator-pole temp gradient (default 60K)
        'delv': 6.,       # lapse rate (default 10K)
        ...
        'equilibrium_t_option': 'Mitchell_Vallis'
    },
...
if __name__ == '__main__':
    for delh in np.array([0.2,0.6]):
        run_exp = exp.derive('tgrad_hs_delh_'+str(delh))
        run_exp.namelist['hs_forcing_nml']['delh'] = delh
        run_exp.run(1, num_cores=NCORES, use_restart=False)
        for i in range(2, 45):
            run_exp.run(i, num_cores=NCORES)  # use the restart i-1 by default
    run_exp = exp.derive('tgrad_hs_delh_'+str(delh)+'_fixros_a_rt3a')
    run_exp.run(1, num_cores=NCORES, use_restart=False)
    for i in range(2, 45):
        run_exp.run(i, num_cores=NCORES)  # use the restart i-1 by default
