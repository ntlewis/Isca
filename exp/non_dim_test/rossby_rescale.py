...
NCORES = 32
RESOLUTION = 'T42', 30  # T42 horizontal resolution, 25 levels in pressure
...
expid = 'Ro_equiv_1_16_omg_1_16_rescale_tau_T42'
...
namelist = Namelist({
    ...
    'hs_forcing_nml': {
        ...
        'ka':   ka*-30.,      # Constant Newtonian cooling timescale (default 40 days)
        'ks':    ks*-2.5,      # Boundary layer dependent cooling timescale (default 4 days)
        'kf':   kf*-0.6,       # BL momentum frictional timescale (default 1 days)

        'do_conserve_energy':   True,  # convert dissipated momentum into heat (default True)
        'sponge':True, 
        'sp_coeff': sp_coeff*-0.5, 
        'sigma_sp': 10. / 1000.
    },
...
#Lets do a run!
#rf = os.path.join(GFDL_DATA,expid+'/restarts/res0010.tar.gz')
if __name__ == '__main__':
    exp.run(1, num_cores=NCORES, use_restart=False, overwrite_data=False)#True, overwrite_data=False, restart_file=rf)
    for i in range(1, 25):
        # if i == 16:
        #     diag = DiagTable()
