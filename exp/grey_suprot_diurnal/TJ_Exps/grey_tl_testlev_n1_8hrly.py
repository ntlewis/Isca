...
expname = lambda om_fact:'GREY_T42L40fr6_TL_om_1_'+str(int(om_fact))+'_a_1_1_ps_1_1_TJ001_n1'
timestep  = 540.#60.#120
...
diag.add_file('atmos_hrly', 8, 'hours', time_units='days')
...
if __name__=="__main__":
    om_facts = np.array([10.,2.,4.,6.,8.,20.,40.,60.,80.,100.,200.])
    for om_fact in om_facts:
        expname_use = expname(om_fact)
        run_exp = exp.derive(expname_use+'_8hrly')
        run_exp.namelist['constants_nml']['omega'] = 2*np.pi / 86400. / om_fact
        run_exp.namelist['titan_gray_rad_nml']['diurnal_period'] = -1 * om_fact
        run_exp.run(1, use_restart=True, num_cores=cores, overwrite_data=False, 
        restart_file='/network/group/aopp/planetary/PLR018_LEWIS_VENUS2/isca_data/'+expname_use+'/restarts/res0015.tar.gz')
        for i in range(2,13):#121):
