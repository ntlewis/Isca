...
expname = lambda om_fact:'TITAN_T21LLEB_dailyrestest60_dt120'
timestep  = 120.#60.#120

# planet params 
diurnal_period = 999
solar_constant = 3.6 * 4.
del_sol        = 1.#0.9#1.4
omega          = 4.56e-6#(-1*diurnal_period)
radius         = 2.576e6 #/ 20.
ps             = 1.45e5 #*10.#
grav           = 1.35
R_d            = 290. 
...
# BL params 
gust        = 0.5 
rough_mom   = 1.e-2#5.e-4#5.e-3
rough_moist = 1.e-2#5.e-4#1.e-5
rough_sens  = 1.e-2#5.e-4#1.e-5
background_m = 0.015
background_t = 0.015
...
## RT params 
tau_zero                = 2.7#1./10. * 2. * 8. *4 * 2.#/2.#/2.#77#.06#.#4.54#4.47 #next change the .9 to a .8
pres_zero               = 0.6*ps#ps/6./2.**np.sqrt(2.) / 2. ** (3./2.) * 2**(4./3.) *8**(4./3.) #* 2.#/ 2**(4./3.)#.1*1e5 /*2.
do_rc_optical_depth     = False 
tau_rc                  = -1. 
pres_rc                 = -1.
linear_tau              = 0.05#*np.sqrt(2)#.85
nonlin_tau_exponent     = 2.#5./3.
lin_tau_exponent        = 2./3.

sw_k1                   = 0.0#141.22/4.#35.167 
sw_k2                   = 0.0#0.0643#0.0613 
do_log_sw = True
logtau_B  = 0.1544#0.11069#0.09980#0.09229
logtau_sw_s =  1.1479#1.5248#1.497#1.897
logtau_0 = 39.57#77.57#40.128#148.9#106#193.7#356.7
logtau_siglev = 0.012#2.47255e-3#0.00022#0.0005761
logtau_pres_zero = ps
logf = 1 - 0.5
...
#exp.inputfiles = [os.path.join(base_dir, 'create_ic/titan_col_log_check_newFlux_50H145_leblevs_ic_T21.nc')]
#exp.inputfiles = [os.path.join(base_dir, 'create_ic/titan_col_log_check_newFlux_ic.nc')]

#Tell model how to write diagnostics
diag = DiagTable()
#diag.add_file('atmos_monthly', 2, 'hours', time_units='days')
...
exp.namelist = namelist = Namelist({
    ...
    'spectral_dynamics_nml': {
        ...
        #'eddy_sponge_coeff':10./86400. * radius**2. / (fourier*spherical),
        #'zmu_sponge_coeff':10./86400. * radius**2. / (fourier*spherical),
        #'zmv_sponge_coeff':10./86400. * radius**2. / (fourier*spherical),
...
#         'file_name':'INPUT/titan_col_log_check_newFlux_50H145_leblevs_ic_T21.nc', 
#         'u_name': 'ucomp', 
#         'v_name': 'vcomp', 
#         't_name': 'temp', 
#         'ps_name': 'ps'
...
if __name__=="__main__":
    ...
    #ds = af.open_experiment('titan_col_log_check_newFlux_50H145_leblevs',50,50,'atmos_monthly.nc',
    #                        variables=['temp'],cal=False)
    #T_ini_sfc = np.squeeze(ds.temp.values[-1,-1])
    #run_exp.namelist['mixed_layer_nml']['tconst'] = T_ini_sfc+1.
    run_exp.run(1, num_cores=cores, use_restart=True, overwrite_data=True ,
                restart_file='/network/group/aopp/planetary/PLR018_LEWIS_VENUS2/isca_data/TITAN_T21LLEB/restarts/res0060.tar.gz')
    for i in range(2,5):
