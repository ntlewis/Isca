...
exp = Experiment('grey_T42L50_rad1_1_om1_4_test_scatter_perp_eq_makenmlsame_exact2_wdefaults_noic', codebase=cb)
...
tau_sw_s       = 0.#2./3. * 0.25 * 160. * tau_lw_s *12./16.  # shortwave optical depth in non-window region 
solar_constant = 1360.                                    # solar constant 
albedo         = 0.27                                     # bond albedo 
omegae         = 7.29e-5                                  # Earth's rotation rate 
radiuse        = 6.4e6                                    # Earth's radius (m)
...
sw_window_flag = False                                     # flag for shortwave window `

#exp.inputfiles = [os.path.join(base_dir,'/home/lewis/research/isca_dir/Isca/exp/grey_suprot/column_ics/ic_files/1bar_0frac_ic.nc')]
...
exp.namelist = namelist = Namelist({
    ...
    'constants_nml':{
        ...
        'omega'   : omegae*0.25, # modified in experiment loop 
        'grav'    : grav, 
        'radius'  : radiuse*1., # modified in experiment loop 
        'rdgas'   : R_d, 
        'PSTD'    : pse * 10., 
        'PSTD_MKS': pse, 
        'kappa'   : R_d / cp,
...
if __name__=="__main__":
    exp.run(1, use_restart=False, num_cores=NCORES, overwrite_data=False)#, restart_file='/home/lewis/research/isca_dir/Isca/src/extra/python/scripts/restart_1bar_0abs.tar.gz')
    for i in range(2,11):
        exp.run(i, num_cores=NCORES, overwrite_data=False)#, use_restart=False)
