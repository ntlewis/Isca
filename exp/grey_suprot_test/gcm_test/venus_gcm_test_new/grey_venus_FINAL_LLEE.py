...
expname = lambda om_fact:'VENUS_T21LLEE'
timestep  = 120.#60.#120
...
rough_moist = 2.e-4
rough_sens  = 2.e-4
background_m = 0.015#5#5
background_t = 0.015#5#5
tj_bl_pres     = 7.e4 * ps / 1.e5 
...
lee_pk = np.flip(np.array([0.0,0.5152,2.973,7.5372,12.459,16.147,17.807,17.446,15.558,12.801,9.7655,7.0307,4.9103,3.3474,
                          2.2229,1.4337,.8960,.5408,.3146,.1759,.0946,.0489,.0245,.0119,.0057,.0027,.0013,.00072,.00041,
                          .00026,.0])) * 1e5 * ps / 95.944e5 
lee_bk = np.flip(np.concatenate([np.array([95.944,86.185,72.483,55.949,39.375,25.107,
                                          14.305,7.0727,2.8270,0.7645,0.0882]), np.zeros(20)])) / 95.944

#ztwid = np.flip(np.arange(levels+1))/levels
#bk = np.exp(-5.*(0.05*ztwid + 0.95*ztwid**3.))
#pk = np.zeros_like(bk)
new_pk = lee_pk.tolist() #(pk + ps * bk).tolist()
new_bk = lee_bk.tolist() #(np.zeros_like(bk)).tolist()
...
exp.inputfiles = [os.path.join(base_dir, 'create_ic/venus_col_logcheck_finalFlux_52H145_leelevs_cp900_ic_T21.nc')]
...
diag.add_field('atmosphere', 'diff_coeff_t',time_avg=True)
...
exp.namelist = namelist = Namelist({
    ...
    'ic_from_external_file_nml':{ 
        'file_name':'INPUT/venus_col_logcheck_finalFlux_52H145_leelevs_cp900_ic_T21.nc', 
        'u_name': 'ucomp', 
        'v_name': 'vcomp', 
        't_name': 'temp', 
        'ps_name': 'ps'
...
if __name__=="__main__":
    ...
    ds = af.open_experiment('venus_col_logcheck_finalFlux_52H145_leelevs_cp900',50,50,'atmos_monthly.nc',
