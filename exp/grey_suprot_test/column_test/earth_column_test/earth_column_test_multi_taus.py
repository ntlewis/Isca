...
tau_sw_s = 0.0# 2./3. * 0.25 * 160. * tau_lw_s *12./16.  #.5#0.22 * tau_lw_s #0.22 is anti-greenhouse parameter 
solar_constant = 1360.
albedo = 0.27
omega = 7.29e-5
radius = 6.371e6
...
#Lets do a run!
tau_sw_s_array = np.array([0.0,0.25,0.5,1.]) * 2./3. * 1./4. * 160. * tau_lw_s * 12./16.
names_array = ['0','1_4','1_2','1']

if __name__=="__main__":

    for n, tau_sw_s in enumerate(tau_sw_s_array):
        run_exp = exp.derive('earth_grey_test_column_tau_sw_s_'+names_array[n])
        run_exp.namelist['two_stream_scatter_rad_nml']['sw_abs_c'] = grav * tau_sw_s / ps
