...
exp = Experiment('titan_col_robinson_tapio_k10k20_seasons', codebase=cb)

#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 933120000.0/360., 'seconds', time_units='days')
...
dt_conv = 3600. #* 2. # slow timescale for dry convection 
gamma_conv = 0.77
...
exp.namelist = namelist = Namelist({
    ...
    'column_nml': {
        'lon_max': 24, # number of columns in longitude, default begins at lon=0.0
        'lat_max': 12, # number of columns in latitude, precise 
                      # latitude can be set in column_grid_nml if only 1 lat used. 
        'num_levels': 36,  # number of levels 
        'initial_sphum': 0.0, 
        'vert_coord_option': 'uneven_sigma',         # default: 'even_sigma'
    ...
    # 'column_grid_nml': { 
    #     'lat_value': 0.#p.rad2deg(np.arcsin(1/np.sqrt(3))) # set latitude to that which causes insolation in frierson p2 radiation to be insolation / 4. 
    ...
    'idealized_moist_phys_nml': {
        ...
        'convection_scheme': 'LLCS', #Use the simple Betts Miller convection scheme 
    },
    ...
    'constants_nml': { # set constants to be those of Venus 
        ...
        'PSTD_MKS': ps, #pa 
        'kappa': R_d / cp,
        'orbital_period':933120000.0, # 30 years
