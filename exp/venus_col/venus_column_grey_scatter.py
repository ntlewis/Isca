...
exp = Experiment('venus_column_scatter_473.56_1.4_0.0_n0.5asym_nocloud', codebase=cb)
...
exp.namelist = namelist = Namelist({
    ...
    'two_stream_scatter_rad_nml': {
        ...
        'lw_chi_sca' : 0.0, # lw scattering crosssection for cloud 
        'sw_chi_sca' : 0.0,#0.6e-5, # sw scattering crosssection for cloud 
        'lw_kappa0' : 473.56,#*2/92.0e5,#9.0e-4, #lw absorption coefficient 
        'sw_kappa0' : 1.4,#12.48,  # sw absorption coefficient (quite different to lw
                                 #coefficient as normalised differently )
        'lw_p_abs_ref' : 1.,#9.2e6, # reference pressure for lw abs coeff normalization
        'sw_p_abs_ref' : 1.,#9.2e6, # reference pressure for lw abs coeff normalization 
        'g_asym' : -0.5, 
        'pcloud' : 1.e5, # pressure centre for cloud 
        'dpcloud' : .5e5, # geometric thickness of cloud 
        'qmax' : 0.,#1.,#5.0e-6, # mmr for cloud droplets 
        'do_seasonal':False,
    ...
    'mixed_layer_nml': {
        ...
        'albedo_value': 0.95,                  #Albedo value used             
    },
