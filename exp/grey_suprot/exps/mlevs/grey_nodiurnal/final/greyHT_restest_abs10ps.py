...
# expname 
expname = lambda x: 'GREY_T42L48_om_1_'+str(int(x))+'_a_1_1_ps_10_1_tau43_withabs_testres' # next 9/10
timestep  = 60.#60.#120
...
om_facts = np.array([4.,8.,16.,64.])#4., 8., 16., 64.])
...
if __name__=="__main__":
    ...
    for om in om_facts: 
        run_exp = exp.derive(expname(om))
        run_exp.namelist['constants_nml']['omega'] = omegae / om 

        if str(int(om)) == '64': 
            run_exp.run(1, use_restart=True, restart_file='/network/group/aopp/planetary/PLR018_LEWIS_VENUS2/isca_data/GREY_T42L48_om_1_'+str(int(om))+'_a_1_1_ps_10_1_tau43_noabs/restarts/res0100.tar.gz', num_cores=cores, overwrite_data=False)
            for i in range(2,61):#121):
                run_exp.run(i, num_cores=cores, overwrite_data=False)
        else:
            run_exp.run(1, use_restart=True, restart_file='/network/group/aopp/planetary/PLR018_LEWIS_VENUS2/isca_data/GREY_T42L48_om_1_'+str(int(om))+'_a_1_1_ps_10_1_tau43_noabs/restarts/res0050.tar.gz', num_cores=cores, overwrite_data=False)
            for i in range(2,31):#121):
                run_exp.run(i, num_cores=cores, overwrite_data=False)
