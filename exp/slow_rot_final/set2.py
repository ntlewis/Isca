...
NCORES = 96
RESOLUTION = 'T127', 80  # T42 horizontal resolution, 25 levels in pressure
...
# inv_omgs =  np.array([1./4., 1./2., 1., 16., 64.])
# omgs = 1 / inv_omgs
# finish = np.ones(len(omgs), dtype=int) * 73
# finish[0:3] = 25

#inv_omgs =  np.array([1./8., 1., 16., 64.])
inv_omgs = np.array([16.])
omgs = 1 / inv_omgs
finish = np.ones(len(omgs), dtype=int) * 109

if __name__ == '__main__':
    for n, omg in enumerate(omgs):
        #if n == 0 or n == 1 or n == 2:
        #    exp_name = 'omg_'+str(int(1/inv_omgs[n]))+'_1_T127L80_FINAL'
        #else:
        exp_name = 'omg_1_'+str(int(inv_omgs[n]))+'_T127L80_FINAL'
        run_exp = exp.derive(exp_name)
        run_exp.namelist['constants_nml']['omega'] = omega_e * omg
        diag = DiagTable()
